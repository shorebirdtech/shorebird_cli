  ...
  group(IosArchiveDiffer, () {
    ...
    group('xcarchive', () {
      group('changedPaths', () {
        ...
        test('finds no differences when only Mach-O UUID differs', () async {
          final fileSetDiff =
              await differ.changedFiles(baseIpaPath, baseChangedUuidPath);
          expect(fileSetDiff, isEmpty);
        });

        test('finds differences between two different xcarchives', () async {
          final fileSetDiff =
              await differ.changedFiles(baseIpaPath, changedAssetXcarchivePath);
          if (platform.isMacOS) {
      ...
      group('changedFiles', () {
        ...
        test('detects dart changes', () async {
          final fileSetDiff =
              await differ.changedFiles(baseIpaPath, changedDartXcarchivePath);
        ...
        test('detects swift changes', () async {
          final fileSetDiff =
              await differ.changedFiles(baseIpaPath, changedSwiftXcarchivePath);
          expect(differ.assetsFileSetDiff(fileSetDiff), isEmpty);
    ...
    group('xcframework', () {
      group('changedPaths', () {
        ...
        test('finds differences between two differed zipped xcframeworks', () async {
          final fileSetDiff =
              await differ.changedFiles(
                baseXcframeworkPath,
