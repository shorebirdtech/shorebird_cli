...
void main() {
  group('scoped', () {
    test('creates instance with default constructor', () {
      final instance = runScoped(() => auth, values: {
        authRef,
  ...
  group(Auth, () {
    ...
    const user = User(id: 42, email: email, jwtIssuer: googleJwtIssuer);
    const scopes = <String>[];
    ...
    group('client', () {
      ...
      group('when token is invalid', () {
        ...
        test(
          'prints warning message when token string is not valid base64',
          () async {
            auth = buildAuth();
            verify(
              () => logger.warn('''
