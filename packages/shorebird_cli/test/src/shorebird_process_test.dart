  ...
  group('ShorebirdProcess', () {
    ...
    group('run', () {
      ...
      test('forwards non-flutter executables to Process.run', () async {
        ...
        verify(
          () => processWrapper.run('git', [
            'pull',
          ], runInShell: true, environment: {}, workingDirectory: '~'),
        ).called(1);
      });
      ...
      test('does not replace flutter with our local flutter if'
        ...
        await runWithOverrides(
          () => shorebirdProcess.run('flutter', [
            '--version',
          ], runInShell: true, workingDirectory: '~', useVendedFlutter: false),
        );

        verify(
          () => processWrapper.run('flutter', [
            '--version',
            '--verbose',
          ], runInShell: true, environment: {}, workingDirectory: '~'),
        ).called(1);
    ...
    group('runSync', () {
      ...
      test('forwards non-flutter executables to Process.runSync', () async {
        runWithOverrides(
          () => shorebirdProcess.runSync('git', [
            'pull',
          ], runInShell: true, workingDirectory: '~'),
        );

        verify(
          () => processWrapper.runSync('git', [
            'pull',
          ], runInShell: true, environment: {}, workingDirectory: '~'),
        ).called(1);
      });
      ...
      test(
        ...
        () {
          ...
          verify(
            () => processWrapper.runSync('flutter', [
              '--version',
              '--verbose',
            ], runInShell: true, environment: {}, workingDirectory: '~'),
          ).called(1);
    ...
    group('start', () {
      ...
      test('forwards non-flutter executables to Process.run', () async {
        ...
        verify(
          () => processWrapper.start('git', [
            'pull',
          ], runInShell: true, environment: {}),
        ).called(1);
      });
